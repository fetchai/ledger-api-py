...
def parse_args(args=None):
    ...
    parser.add_argument('-H', '--host-name', type=str, help='Host name of the node where transaction is supposed to be committed. The hostname  is in following format `host[:port]`, for example `localhost:8000`, where `:port` fraction is not madatory and defaults to `:8000` if not provided.', default="localhost:8000")

    parser.add_argument('-V', '--host-api-version', type=str, help='Version of host API.', default="1")
