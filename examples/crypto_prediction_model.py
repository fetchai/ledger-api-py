...
def main():
    ...
    # make a prediction
    current_prediction = contract.action(api, 'makePrediction')
    print("current historics: " + current_prediction)

    # update the graph with a new model
    with open(GRAPH_FILE_NAME, mode='rb') as file:
        graph_from_file = file.read()
        api.sync(contract.action(api, 'updateGraph', fet_tx_fee, [entity1], graph_from_file))
